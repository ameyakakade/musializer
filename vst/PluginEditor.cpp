...
shaderC::shaderC(AudioPluginAudioProcessor& p) : processorRef(p)
{
    fragmentSource =
        ...
        vec4 getSmear(float radius, vec2 barTop, vec2 fragC, float window, float non)
        {
            float r = radius*window;
            float power = 2.0;
            vec2 p = fragC - barTop;
        ...
        vec4 drawCircle(vec2 uv, int offset)
        {
            ...
            return getCircle(0.5, barT, uv, cell_width*5*sqrt(data.x), data.z);
        }

        vec4 drawSmear(vec2 uv, int offset)
        {

            vec4 data = getBarDataS(uv, offset);

            vec2 barT = vec2(data.z + (cell_width/2), data.x);

            return getSmear(0.5, barT, uv, cell_width*5*sqrt(data.x), data.z);
        }
